...
```{r}
m_wv <- lm(CASUALTY ~ WEATHER * VISIBLTY, data=df)
anova(m_wv)
```
None of these factors showed statistical significance in an ANOVA 

```{r}
adverse_levels <- c("Rain","Snow","Fog","Sleet","Storm")
risky_vis <- c("Dark","Dawn")
dc2 <- dc %>% mutate(RiskZone = (WEATHER %in% adverse_levels) & (VISIBLTY %in% risky_vis))
t_c <- t.test(CASUALTY ~ RiskZone, data=dc2, alternative="greater")
t_c
```
