...
df$TYPE <- factor(df$TYPE, labels = c("Derailment", "HeadOn", "Rearend", "Side",
                                      "Raking", "BrokenTrain", "Hwy-Rail",
                                      "GradeX", "Obstruction", "Explosive",
                                      "Fire","Other", "SeeNarrative" ))

# Recode Categorical 'TypeQ'
df$TYPEQ <- factor(df$TYPEQ, labels = c("NA", "Freight", "Passenger",
                                        "Commuter", "Work", "Single",
                                        "CutofCars", "Yard", "Light", "Maint",
                                        "MaintOfWay", "Passenger", "Commuter", "ElectricMulti", "ElectricMulti"))

# Recode Categorical 'Weather'
df$WEATHER <- factor(df$WEATHER, labels = c("Clear", "Cloudy", "Rain", "Fog",
                                            "Sleet", "Snow"))

# Recode Categorical 'Method'
df$METHOD <- factor(as.integer(df$METHOD), 
                    levels = 1:16, labels = c("ATCS", "Auto Rrain Control",
                    "Auto Train Stop", "Cab Signals", "Traffic Control",
                    "Interlocking", "Automatic Block Rules", "Current of
                    Traffic", "Time/Table/Train Orders", "Track Warrant
                    Control", "Direct Traffic Control", "Yard Limits", "Special
                    Instructions", "Other than Main Track", "Other", "Positive
                    Train Control"))
...
vars <- c("CASUALTY", "ACCDMG", "TYPE", "TYPEQ", "Cause", "WEATHER", "VISIBLTY",
          "TEMP", "TRNSPD", "METHOD", "TRKCLAS")
...
2: Train Speed (TRNSPD) is associated with a steeper increase in damages for Human and Miscellaneous Caused Accidents
...
```
How did we reach this hypothesis?
This hypothesis was motivated by both theoretical reasoning and patterns observed in preliminary data exploration. We observed that accidents caused by human error (CauseH) or miscellaneous causes (CauseM) tended to occur at a wider range of train speeds compared to other causes.
...
**2. Lower Class tracks are associated with more casualties**
...
```
None of these interaction terms are significant at the p = 0.05 level. This goes against our overall hypothesis, namely in that the train-speed:cause interaction is associated with more damage.
...
**Build Models - Main Effects Model**
```{r}
c_main <- lm(CASUALTY ~ Cause + TYPE + WEATHER + VISIBLTY + TRKCLAS + TRNSPD + ACCDMG, df)
c_step <- stepAIC(c_main, trace=0)
summary(c_step)
```
...
**Determine necessity for y-variable transform**
```{r}
bc_c <- boxcox(CASUALTY ~ Cause + TYPE + WEATHER + VISIBLTY + TRKCLAS + TRNSPD + ACCDMG, data=as.data.frame(df))
lambda_opt_c <- bc_c$x[which.max(bc_c$y)]
lambda_opt_c
```

**Transform Response Variable (use for subsequent models)**
```{r}
df$CASUALTY_bc <- (df$CASUALTY ^ lambda_opt - 1) / lambda_opt
```

# Refit 
```{r}
c_main <- lm(CASUALTY_bc ~ Cause + TYPE + WEATHER + VISIBLTY + TRKCLAS + TRNSPD + ACCDMG, df)
c_step <- stepAIC(c_main, trace=0)
summary(c_step)
```
...
**Interaction Model**
```{r, echo=FALSE}
c_inter <- lm(CASUALTY_bc ~ (Cause + VISIBLTY + TRKCLAS + TRNSPD + 
    ACCDMG)^2, df)
c_step <- stepAIC(c_inter, trace=0)
summary(c_inter)
```
In the interaction model, we see that TRNCLAS^10:ACCDMG is significant at the 0 level and as TRKCLAS#:ACCDMG increases, the factor becomes more significant. It is notable that the TRKCLAS#:ACCDMG has a negative coefficient. The interaction term TRNSPD:ACCDMG is significant at the 0.001 level, suggesting that there is some association between train speed, damage, and number of casualties. The R^2 for the interaction model is also an order of magnitude higher than that of the main effects model, suggesting that it has stronger predictive power.

**Comparing the Models**
```{r}
AIC(c_main, c_inter)
anova(c_main, c_inter)
```

**LASSO Regression ** 
```{r}
# Build matrix for main effects 
x_c <- model.matrix(CASUALTY_bc ~ (Cause + TYPE + WEATHER + VISIBLTY + TRKCLAS + TRNSPD + ACCDMG)^2, data=df)[,-1]
y_c <- df$CASUALTY_bc
```
...
```{r}
# Run Lasso Regression, Plot Results
lasso_cv_c <- cv.glmnet(x_c, y_c, alpha = 1, nfolds = 10)
plot(lasso_cv_c)
```
...
```{r, echo=FALSE}
# What were the important factors?
min_cverr <- coef(lasso_cv_c, s = "lambda.min")
coef_min <- data.frame(
  factor = rownames(min_cverr),
  estimate = as.numeric(min_cverr)
)
...
```{r}
set.seed(42)
idx  <- createDataPartition(y_c, p = 0.8, list = FALSE)
x_tr <- x_c[idx, ];  y_tr <- y_c[idx]
x_te <- x_c[-idx,];  y_te <- y_c[-idx]

# Cross-validate ONLY on training
lasso_cv_tr <- cv.glmnet(x_tr, y_tr, alpha = 1, standardize = TRUE)

# Predict with chosen lambda
pred_min  <- predict(lasso_cv_tr, newx = x_te, s = "lambda.min")
pred_1se  <- predict(lasso_cv_tr, newx = x_te, s = "lambda.1se")

rmse <- function(a,b) sqrt(mean((a-b)^2))
r2   <- function(y, yhat) 1 - sum((y - yhat)^2)/sum((y - mean(y))^2)

metrics_min <- c(RMSE = rmse(y_te, pred_min), R2 = r2(y_te, pred_min))
metrics_1se <- c(RMSE = rmse(y_te, pred_1se), R2 = r2(y_te, pred_1se))
rbind(lambda_min = metrics_min, lambda_1se = metrics_1se)
```
This R^2 is slightly better than the interaction model that was achieved with stepwise regression. We could go further and tune the other parameters, but that is outside the scope of the class. 

## HYPOTHESIS TESTING
**Weather x Visibility**
```{r}
m_wv <- lm(CASUALTY_bc ~ WEATHER * VISIBLTY, data=df)
anova(m_wv)
```
Neither weather nor visibility alone (nor their interaction) significantly explained variation in casualties (ANOVA p > 0.05).

```{r}
adverse_levels <- c("Rain","Snow","Fog","Sleet")
risky_vis <- c("Dark","Dawn")
df2 <- df %>% mutate(RiskZone = (WEATHER %in% adverse_levels) & (VISIBLTY %in% risky_vis))
t_c <- t.test(CASUALTY_bc ~ RiskZone, data=df2, alternative="greater")
t_c
```
A two-sample t-test comparing transformed casualties between normal and risky conditions (adverse weather × low visibility) found no significant difference in means, t= –0.62, p = 0.73.
The average casualties were slightly higher under risky conditions (M = 0.362) than under normal conditions (M = 0.337), but this difference was not statistically significant.

**Track Class and Casualties**
Treating as ordinal 
```{r}
m_H4 <- lm(CASUALTY ~ TRKCLAS, data = df)
summary(m_H4)
```
Treating as Factors 
```{r}
data$TRKCLAS <- factor(data$TRKCLAS)  # not ordered
m_cat <- lm(CASUALTY ~ TRKCLAS, data = data)
summary(m_cat)
```
Track Classes 7 and 8 are associated with increases in casualties at the 0.001 significance level. This goes against our initial interpretation of track class. We thought that higher class meant better tracks, but we through some research, we found that higher classes just allow for higher speed limits. This makes sense given that the higher track classes are actually associated with more casualties rather than less. This is contradictory to our initial understanding. Therefore, we are not able to reject the null hypothesis. 

See: https://www.jgmes.com/webstart/library/table_fra_track.htm

# PART 4: EVIDENCE AND RECOMMENDATIONS
**Hypothesis Conclusions**
These hypotheses were evaluated in detail in their respective sections; here, we provide a concise summary of the key findings and implications.

H1: We found that the true difference in mean damages between CauseH (human causes) and other causes is greater than zero. This confirms our hypothesis that human-caused accidents tend to result in greater damage.

H2: None of the interaction terms between Cause and TRNSPD were statistically significant in the general model. A one-sided test suggested that the interaction adds some explanatory power, providing weak support for our hypothesis. However, because the two-sided test was not significant, this could be interpreted as p-hacking. Interestingly, the slope of the interaction was greatest for CauseM (miscellaneous) rather than CauseH (human), contrary to our expectations. This suggests that accidents attributed to miscellaneous causes exhibited a larger increase in damages as train speed increased.

H3: We failed to reject the null hypothesis. Although adverse or risky weather conditions were associated with a slightly higher number of casualties on average, this difference was not statistically significant.

H4: We failed to reject the null hypothesis. Higher track class was unexpectedly associated with more casualties, rather than fewer. This relationship was significant at the 0.001 level for class 7 and 8 only.

Policy Recommendations/Implications:
Based on these findings, we recommend that the Federal Railroad Administration (FRA) prioritize measures aimed at reducing human-caused accidents. Specifically, the FRA could implement enhanced operator training programs, improved speed monitoring systems, stricter speed limits in high-risk zones, and shorter shift duration to mitigate fatigue-related errors.
